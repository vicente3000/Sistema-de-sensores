...
export const getThreshold = async (req, res) => {
    ...
    if (!doc)
        throw new HttpError(404, 'Threshold not found');
    return res.json(ok(doc));
};
export const upsertThreshold = async (req, res) => {
    const { sensorId } = req.params;
    const sensorExists = await Sensor.exists({ _id: sensorId });
    if (!sensorExists)
        throw new HttpError(404, 'Sensor not found');
    const { min, max, hysteresis } = req.body;
    const doc = await Threshold.findOneAndUpdate({ sensorId }, { sensorId, min, max, hysteresis: hysteresis ?? 0 }, { new: true, upsert: true, setDefaultsOnInsert: true }).lean();
