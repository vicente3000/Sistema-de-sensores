...
export const deletePlant = async (req, res) => {
    ...
    if (sensorIds.length)
        await Threshold.deleteMany({ sensorId: { $in: sensorIds } });
    return res.json(ok({ deleted: true }));
};
// Ruta de compatibilidad con payload legado { plant, sensors[] }
export const legacyAddPlant = async (req, res) => {
    const { plant, sensors } = req.body;
    if (!plant || !plant.name)
        throw new HttpError(400, 'El nombre de la planta es obligatorio');
