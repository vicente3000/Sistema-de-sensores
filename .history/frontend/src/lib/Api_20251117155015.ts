...
export async function createSensor(
  plantId: string,
  input: { type: SensorDoc["type"]; unit?: string }
) {
...
export async function deleteSensor(sensorId: string) {
  return http<{ deleted: true }>(v1(`/sensors/${sensorId}`), {
    method: "DELETE",
  });
}
...
export async function upsertThreshold(
  sensorId: string,
  input: { min: number; max: number; hysteresis?: number }
) {
