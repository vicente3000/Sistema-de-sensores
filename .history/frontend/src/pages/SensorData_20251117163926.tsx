...
import { listPlants, listSensors } from "../lib/Api";
...
export default function SensorData() {
  ...
  const [rangePreset, setRangePreset] = useState<"1h"|"6h"|"24h"|"7d"|"custom">("6h");
  ...
  // Cargar plantas al inicio (vía cliente API para compatibilidad con Docker)
  useEffect(() => {
    const loadPlants = async () => {
      try {
        const res = await listPlants({ limit: 200 });
        setPlants(res.items);
      } catch (err: any) {
  ...
  useEffect(() => {
    ...
    const loadSensors = async () => {
      if (!plant) return;
      try {
        const res = await listSensors(plant, { limit: 200 });
        setSensors(res.items as any);
      } catch (e) {
  ...
  const loadAgg = async () => {
    ...
    try {
      ...
      if (rangePreset !== "custom") {
        ...
        if (rangePreset === "1h") fromDate = new Date(now.getTime() - 1*60*60*1000);
        if (rangePreset === "6h") fromDate = new Date(now.getTime() - 6*60*60*1000);
        if (rangePreset === "24h") fromDate = new Date(now.getTime() - 24*60*60*1000);
        if (rangePreset === "7d") fromDate = new Date(now.getTime() - 7*24*60*60*1000);
  ...
  return (
    <section>
      ...
      <div className="filters-row-top">
        {/* Selector simple de planta */}
        <div className="fieldset fieldset-small">
          <h3 className="subtitle">Planta</h3>
          <label>
            <span className="label-caption">Filtrar</span>
            <input
              placeholder="Nombre..."
              value={plantSearch}
          ...
          <label>
            <span className="label-caption">Selecciona</span>
            <select
              value={plant}
              onChange={(e) => {
        ...
        {/* Pestañas rápidas de tipo de sensor */}
        <div className="fieldset fieldset-small">
          <h3 className="subtitle">Sensor</h3>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {(["humidity","ph","temp","lux"] as SensorType[]).map((t) => {
              const enabled = sensors.some(s => s.type === t);
              return (
                <button
                  key={t}
                  type="button"
                  className={`pill ${sensor===t ? 'pill-active' : ''}`}
                  disabled={!plant || !enabled}
                  onClick={() => setSensor(t)}
                >
                  {t.toUpperCase()}
                </button>
        ...
        {/* Presets de rango y controles */}
        <div className="fieldset fieldset-wide">
          <h3 className="subtitle">Rango y promedio</h3>
          <div className="filters-inner">
            ...
            <label>
              ...
              <select value={rangePreset} onChange={(e)=>setRangePreset(e.target.value as any)}>
                <option value="1h">Últ. 1h</option>
                <option value="6h">Últ. 6h</option>
                <option value="24h">Últ. 24h</option>
                <option value="7d">Últ. 7d</option>
      ...
      <div className="filters-row-bottom">
        <div className="fieldset fieldset-full">
          <h3 className="subtitle">Resumen diario</h3>
          <p className="note">
