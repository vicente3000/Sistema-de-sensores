...
// alertas
export async function listAlerts(params?: { plantId?: string; sensorId?: string; from?: string; to?: string; level?: 'normal'|'grave'|'critica'; status?: 'pendiente'|'en_progreso'|'completado'; limit?: number }) {
...
export async function updateAlertStatus(id: string, status: AlertStatus) {
  return http(v1(`/alerts/${id}/status`), {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
