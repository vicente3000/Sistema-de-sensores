...
describe("Aggregated history and daily aggregates", () => {
  ...
  it("GET /sensors/history/agg returns aggregated buckets", async () => {
    const row = (ts: Date, v: number) =>
      ({ get: (k: string) => (k === "ts" ? ts : v) } as any);
    // prepare rows across a small range to build a few buckets
    fakeClient.execute.mockResolvedValueOnce({
      rows: [
    ...
    const res = await request(app)
      .get("/api/v1/sensors/history/agg")
      .query({
        plant: "p1",
        sensor: "humidity",
        step: "1m",
        from: "2025-11-03T00:00:00Z",
        to: "2025-11-03T00:05:00Z",
      });
  ...
  it("GET /sensors/daily returns daily aggregates", async () => {
    ...
    fakeClient.execute.mockResolvedValueOnce({
      rows: [makeDaily(1, 2.5, 4, 10)],
    } as any);
    // next calls empty
    fakeClient.execute.mockResolvedValue({ rows: [] });

    const res = await request(app)
      .get("/api/v1/sensors/daily")
      .query({
        plant: "p1",
        sensor: "humidity",
        from: "2025-11-03T00:00:00Z",
