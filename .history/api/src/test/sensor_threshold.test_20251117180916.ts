...
describe("Sensors & Thresholds", () => {
  ...
  beforeAll(async () => {
    ...
    process.env.MONGO_URI =
      "mongodb://localhost:27017/greendata_test_sensor_threshold";
    await connectMongo();
  ...
  it("create sensor, upsert/get/delete threshold", async () => {
    ...
    // Get threshold
    const getTh = await request(app).get(
      `/api/v1/sensors/${sensorId}/threshold`
    );
    expect(getTh.status).toBe(200);
    expect(getTh.body.data.max).toBe(70);

    // Delete threshold
    const delTh = await request(app).delete(
      `/api/v1/sensors/${sensorId}/threshold`
    );
